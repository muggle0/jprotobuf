...
public class RequrieRepeatedDojoClassWithLargeIndexTest {
    ...
    private byte[] decodeByprotobuf() throws IOException {
        ...
        RequrieRepeatedTypeWithLargeIndex.InterClassName icn = RequrieRepeatedTypeWithLargeIndex.InterClassName.newBuilder().addList("hello").addList("world").build();
        return icn.toByteArray();
    }
    
    private void encodeByprotobuf(byte[] byteArray) throws IOException {
        RequrieRepeatedTypeWithLargeIndex.InterClassName icn = RequrieRepeatedTypeWithLargeIndex.InterClassName.parseFrom(byteArray);
